...
public class RecipeCompilerTest {
  ...
  public void testByteSize() throws Exception {
    String recipe = "set-column :size_in_kb 1.5KB;";
    RecipeCompiler compiler = new RecipeCompiler();
    CompileStatus status = compiler.compile(recipe);

    Assert.assertFalse("Recipe with byte size should compile successfully", !status.isSuccess());
  }

  @Test
  public void testTimeDuration() throws Exception {
    String recipe = "set-column :duration 200.5ms;";
    RecipeCompiler compiler = new RecipeCompiler();
    CompileStatus status = compiler.compile(recipe);

    Assert.assertFalse("Recipe with time duration should compile successfully", !status.isSuccess());
  }

  @Test
  public void testByteUnitCaseInsensitivity() throws Exception {
    String recipe = "set-column :size1 100kb; set-column :size2 200KB;";
    RecipeCompiler compiler = new RecipeCompiler();
    CompileStatus status = compiler.compile(recipe);

    Assert.assertFalse("Recipe with byte size unit in different cases should compile successfully", !status.isSuccess());
  }

  @Test
  public void testMultipleUnitsInOneRecipe() throws Exception {
    String recipe = "set-column :small_file 256KB;" +
        "set-column :large_file 1.5GB;" +
        "set-column :quick_response 100ms;" +
        "set-column :slow_response 2.5s;";

    RecipeCompiler compiler = new RecipeCompiler();
    CompileStatus status = compiler.compile(recipe);

    Assert.assertFalse("Recipe with multiple size and time units should compile successfully", !status.isSuccess());
  }
  ...
  public void testAggregateStatsDirective() throws Exception {
    ...
    Assert.assertFalse("AggregateStats directive should compile successfully", !status.isSuccess());
  }

  @Test
  public void testAggregateStatsWithUnits() throws Exception {
    String recipe = "aggregate-stats :data_transfer_size :response_time total_size_kb total_time_ms KB ms;";
    RecipeCompiler compiler = new RecipeCompiler();
    CompileStatus status = compiler.compile(recipe);

    Assert.assertFalse("AggregateStats directive with units should compile successfully", !status.isSuccess());
  }
}
